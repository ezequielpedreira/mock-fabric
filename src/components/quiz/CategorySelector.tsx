...
export function CategorySelector({ language, onSelectCategory }: CategorySelectorProps) {
  ...
  const labels: Record<string, Record<Language, string>> = {
    ...
    allDesc: {
      "pt-br": "Pratique com todas as questões disponíveis de todos os domínios",
      "en-us": "Practice with all available questions from every domain",
      es: "Practica con todas las preguntas disponibles de todos los dominios",
      fr: "Pratiquez avec toutes les questions disponibles de tous les domaines",
    },
    questionsLabel: {
      "pt-br": "questões",
      "en-us": "questions",
          ...
          <CardContent className="p-4 sm:p-5 space-y-3">
            <div className="flex items-start justify-between gap-2">
              ...
              <Badge variant="secondary" className="shrink-0">
                {totalQuestions} {labels.questionsLabel[language]}
              </Badge>
            </div>
            <div className="min-w-0">
              <h3 className="font-semibold text-sm sm:text-base text-foreground group-hover:text-primary transition-colors break-words">
                {labels.allQuestions[language]}
              </h3>
              <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                {labels.allDesc[language]}
              </p>
            </div>
                ...
                <CardContent className="p-4 sm:p-5 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Badge variant="outline" className="shrink-0 text-xs">
                      {count} {labels.questionsLabel[language]}
                    </Badge>
                  </div>
                  <div className="min-w-0">
                    <h3 className="font-semibold text-sm sm:text-base text-foreground group-hover:text-primary transition-colors break-words">
