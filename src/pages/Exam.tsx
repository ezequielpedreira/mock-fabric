...
function shuffleAnswers(qs: Question[]): Question[] {
  return qs.map((q) => {
...
const Exam = () => {
  const { language, setLanguage, allQuestions, questionsLoading, recordAnswer, completeQuiz } =
    useQuiz();
  ...
  }, [allQuestions, examQuestions.length, questionsLoading]);

  useEffect(() => {
    if (finished || examQuestions.length === 0) return;
  ...
  }, [examQuestions.length, finished, completeQuiz]);
  ...
  const goToQuestion = useCallback(
    ...
    [examQuestions, localAnswers],
  );

  const next = useCallback(() => {
    if (currentIndex < examQuestions.length - 1) goToQuestion(currentIndex + 1);
  }, [currentIndex, examQuestions.length, goToQuestion]);
    ...
    return (
      <div className="fabric-ambient flex h-screen flex-col overflow-hidden">
        <Header
          ...
          title="Modo Prova — DP-600"
          onBack={() => navigate("/")}
        />
        <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground">
          <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Carregando banco de questões...
        </div>
      </div>
    );
  }
    ...
    <div className="fabric-ambient flex h-screen flex-col overflow-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/70 bg-card/85 px-3 py-2 backdrop-blur-xl sm:px-6 sm:py-3">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span
            className={`font-mono font-bold text-base sm:text-lg ${timeLeft < 300 ? "text-destructive" : "text-foreground"}`}
          >
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          <span className="text-xs sm:text-sm text-muted-foreground">
            {localAnswers.length}/{examQuestions.length}
